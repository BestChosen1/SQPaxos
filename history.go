...
//增加历史记录项，方法参数（key,input,output,start,end)
...
////linearizable:并发检查历史记录的每个分区是否可线性化，并返回异常读取的总数
...
//WriteFile：把操作写进path所指的file
func (h *History) WriteFile(path string) error {
	//创建一个名为name(path.csv)的文件
...
// ReadFile reads csv log file and create operations in history 读文件
func (h *History) ReadFile(path string) error {
	...
	// NewReader(r io.Reader) *Reader :NewReader函数返回一个从r读取的*Reader
