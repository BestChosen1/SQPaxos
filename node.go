...
//从socket中接收信息，并传递到信息通道中
func (n *node) recv() {
	for {
		m := n.Recv()
		switch m := m.(type) {
			//接收的信息是request，建立一个reply通道，使用通道，n将回复信息返回给reply结点
		case Request:
...
func (n *node) handle() {
	for {
		//从messageChannel中取出信息msg，确定msg的内容，获取内容的类型，
		msg := <-n.MessageChan
		v := reflect.ValueOf(msg)
		name := v.Type().String()
		//判断是否有处理这种类型信息的方法,如果有返回该方法f
